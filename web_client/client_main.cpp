...
static std::string g_identity_json;
...
static void notify_page(const std::string& message) {
    EM_ASM({
        if (window.cm5audioServerMessage) window.cm5audioServerMessage(UTF8ToString($0));
    }, message.c_str());
}

EM_BOOL on_ws_open(int, const EmscriptenWebSocketOpenEvent*, void*) {
    const std::string hello = g_identity_json.empty()
        ? "{\"type\":\"hello\",\"input_channels\":1,\"output_channels\":2,\"sample_rate\":48000,\"format\":\"f32\"}"
        : g_identity_json;
    if (g_ws > 0) emscripten_websocket_send_utf8_text(g_ws, hello.c_str());
...
EM_BOOL on_ws_message(int, const EmscriptenWebSocketMessageEvent* event, void*) {
    if (event->isText) {
        notify_page(std::string(reinterpret_cast<const char*>(event->data), event->numBytes));
    } else if (event->numBytes > 0) {
        g_playback_rb.write(event->data, event->numBytes);
    }
    return EM_TRUE;
}

EM_BOOL on_ws_error(int, const EmscriptenWebSocketErrorEvent*, void*) {
    std::cerr << "[WASM Audio] WebSocket error" << std::endl;
    return EM_TRUE;
}

EM_BOOL on_ws_close(int, const EmscriptenWebSocketCloseEvent*, void*) {
    g_ws = 0;
    std::cout << "[WASM Audio] Closed" << std::endl;
    return EM_TRUE;
}
...
int start_web_audio(const char* ws_url, const char* identity_json) {
    if (g_audio_running) return 0;
    g_identity_json = identity_json ? identity_json : "";
...
EMSCRIPTEN_KEEPALIVE
void set_web_audio_identity(const char* identity_json) {
    g_identity_json = identity_json ? identity_json : "";
    if (g_ws > 0 && !g_identity_json.empty())
        emscripten_websocket_send_utf8_text(g_ws, g_identity_json.c_str());
}
